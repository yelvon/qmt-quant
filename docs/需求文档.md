...
> **实现真源（2026-08-18）**：本文保留产品演进与历史目标，不再作为当前命令、目录和引擎能力的操作手册。当前实现与安全边界以 [AGENTS.md](../AGENTS.md)、[README.md](../README.md) 和 [phase7-nautilus.md](./phase7-nautilus.md) 为准。
...
> **历史设计章节**：下述「Nautilus 最终验证」是早期目标，不代表当前完成度。当前默认验证器是统一 A 股规则内核；Nautilus 仅为显式实验 MVP，失败不 fallback。
...
> **历史架构章节**：本章的 SQLite、目录和 Nautilus 主路径图已过时。当前为 PostgreSQL + flat Parquet，`core/backtest` 统一策略插件/实验产物，正确结构见 [AGENTS.md §3–§6](../AGENTS.md#3-仓库结构)。
...
> **历史目标，尚未完整实现**：当前 Nautilus 仅支持 `ma_cross`、日线、SIM 简化撮合，最多载入 10 标的但策略只绑定首标的；不提供完整 A 股规则、逐日净值/回撤/成交指标，且绝不 fallback。详见 [phase7-nautilus.md](./phase7-nautilus.md)。
...
```bash
# Step 1: 快速研究（当前 CLI）
python -m qmt_quant.cli research run --strategy ma_cross --sector 沪深A股 --range-preset 3y

# Step 2: 默认统一 A 股规则验证
python -m qmt_quant.cli validate run --from-run <research_run_id> --engine custom

# Step 3: 仅在明确接受 MVP 限制时显式运行 Nautilus 实验
python -m qmt_quant.cli validate run --from-run <research_run_id> --engine nautilus
...
python -m qmt_quant.cli screen backtest --run-id <screen_run_id> --engine vectorbt
python -m qmt_quant.cli screen backtest --run-id <screen_run_id> --engine validate
```
...
> **历史数据模型章节**：标题与部分路径仍是 v0.2 设计；当前结构化存储为 PostgreSQL，flat Catalog 为 `data/catalog`，Nautilus 实验 Catalog 为 `data/catalog_nt`。表与迁移以 `migrations/`、配置以 [README](../README.md) 为准。
